street_normalization <- function(vec_address){
  vec_address <- str_replace_all(vec_address, ',','')
  vec_address_base <- copy(vec_address)
  #ind_city <- which(str_detect(vec_address, 'Stockton'))
 # if(!is.na(ind_city)){
 #   vec_address <- vec_address[1:(ind_city-1)]
 # }
  if(length(vec_address)>1){
    ...
    if((str_detect(vec_address[1], paste0('([0-9]{1,', length_prefix, '}|[A-Z]( |\\/))'))==TRUE)){
    ...
    if(nrow(dt_address[!is.na(suffix_ind_lead)][potential_class %in% 'suffix'])==0){
    ...
    dt_address_suffix <- dt_address[class=='suffix']
    dt_address_suffix <- dt_abbrev_suffix[dt_address_suffix]
    dt_address_suffix <- dt_address_suffix[, value:=clean_value]
    ...
    if(suffix_test==TRUE){
      ...
      suffix_ind_max <- nrow(dt_address)-2
    }
    ...
    if(nrow(dt_address[class %in% 'street_name']) < 2){
      if(suffix_test==TRUE){
        street_upper_bound_ind <- min(dt_address[class=='suffix']$ind)
      } else {
        if(unit_test==TRUE){
          street_upper_bound_ind <- min(dt_address[class=='unit']$ind)
        }
        else {
          street_upper_bound_ind <- nrow(dt_address) + 1
        }
      }
      if(street_num_test==TRUE){
        street_lower_bound_ind <- max(dt_address[class=='street_number']$ind)
      } else {
        street_lower_bound_ind <- 0
      }
      dt_address <- dt_address[(is.na(class) | class=='street_name') &
                                 ind < street_upper_bound_ind &
                                 ind > street_lower_bound_ind, class:='street_name']
    }
    dt_address_street_name <- dt_address[class=='street_name']
    ...
    dt_address_street_name <- dt_abbrev_street_name[dt_address_street_name]
    dt_address_street_name <- dt_address_street_name[is.na(clean_value) & class %in% 'street_name', clean_value:=search_upper]
    dt_address_street_name <- dt_address_street_name[, value:=clean_value]
    dt_address <- rbindlist(list(dt_address[!(class %in% 'street_name')], dt_address_street_name), use.names = TRUE, fill=TRUE)
    setorder(dt_address, ind)
    dt_address <- dt_address[, .(ind, value, clean_value, potential_class, class)]
    
    setorder(dt_address, ind)
    normalized_street <- paste0(dt_address[class %in% c('street_name', 'suffix')]$clean_value, collapse = ' ')
    
    # Clean highway
    hwy_replace <- str_to_upper(dt_abbrev[class=='highway']$return[1])
    normalized_street <- str_replace_all(normalized_street, regex_highway, hwy_replace)
  } else {
